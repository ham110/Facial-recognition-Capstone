...
def webcam_face_recognizer(database):
    ...
    while vc.isOpened():
        ...
        if ready_to_detect_identity:
            img, identities = process_frame(img, frame, face_cascade)

        if(identities): #not empty list
            return identities
        key = cv2.waitKey(100)
...
def who_is_it(image, database, model):
    ...
    if min_dist > 0.70: #0.52 before!! 0.625 is 'optimal'
        return None
    else:
...
if __name__ == "__main__":
    ...
    identity = webcam_face_recognizer(database)
    print(identity)
    # print(time.time() - start)
